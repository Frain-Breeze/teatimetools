...
bool Tea::FileMemory::write_endian(uint8_t* data, size_t size, Endian endian) {
	...
	if(_offset + size > _size) {
		...
		uint8_t* new_buffer = (uint8_t*)realloc(_buffer, _offset + size);
...
bool Tea::FileMemory::seek(int64_t pos, Tea::Seek mode) {
	...
	if(mode == Tea::Seek_current) {
		...
		if(_offset + pos > _size) {
			if(_owner) {
				uint8_t* new_buffer = (uint8_t*)realloc(_buffer, _offset + pos);
				if(!new_buffer) { return false; }
				_buffer = new_buffer;
				_size = _offset + pos;
			}
		...
		if(pos > _size) {
			if(_owner) {
				uint8_t* new_buffer = (uint8_t*)realloc(_buffer, pos);
				if(!new_buffer) { return false; }
				_buffer = new_buffer;
				_size = pos;
			}
